...
ConnectionHandler.prototype.createConnection = function (incoming) {
  this.tracer.debug('[websockets] connection to approuter established');
  var req = incoming.upgradeReq;
  var app = req.app;
  this.memoryStore = app.get('memoryStore');
  ...
  var self = this;
  var cookieParserFunction = cookieParser(app.get('sessionCookieKey'));

  cookieParserFunction(req, null, function () {
    self.sessionID = req.signedCookies[app.get('cookieName')];
    self.memoryStore.get(self.sessionID, function (err, sess) {
      req.session = sess;
...
function resetSocketTimeout() {
  ...
  this.timer = setTimeout(this.socketTimeoutCb, this.memoryStore.getDefaultSessionTimeout() * 60 * 1000);
}
...
function connectToBackend(targetUrl, incoming) {
  ...
  try {
    ...
    outgoing = new ws(url, {
      ...
      headers: getHeaders(req, null, targetUrl.destination)
    });
  } catch (err) {
